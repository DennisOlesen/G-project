...
fun evalNot (BVal (Log b), pos) = BVal (Log (not(b)))
  | evalNot (v, pos) =
  raise Error( "Not: argument type is not a boolean. " ^
                "Arg: " ^ pp_val v, pos)
...
and evalExp ( Literal(lit,_), vtab, ftab ) = lit
  ...
  | evalExp ( Times(e1, e2, pos), vtab, ftab ) =
        let val res1   = evalExp(e1, vtab, ftab)
            val res2   = evalExp(e2, vtab, ftab)
        in
            evalBinop(op *, res1, res2, pos)
        end
  | evalExp ( Div(e1, e2, pos), vtab, ftab ) =
        let val res1   = evalExp(e1, vtab, ftab)
            val res2   = evalExp(e2, vtab, ftab)
        in
            evalBinop(op Int.div, res1, res2, pos)
        end
  ...
  (**********************************************************)
  (*** special functions such as map                      ***)
  (**********************************************************)
  | evalExp ( Map ((fid,signat), arrexp, pos), vtab, ftab ) =
        let val map_res_tp = typeOfExp( Map ((fid,signat), arrexp, pos) )
            val arrval = evalExp(arrexp, vtab, ftab)
            val (arr,shape,strides) =
                    case arrval of
                      Arr(a,sh,sp,tp) => (a,sh,sp)
                    | _  => raise Error("in Map: Second Argument " ^
                                        "Is Not An Array: "^pp_val arrval, pos )
            val fexp = SymTab.lookup fid ftab
        in case fexp of
             NONE   => raise Error("in Map: Function "^fid^" Is Not In SymTab!", pos)
           | SOME f => let val atps   = map ( fn (Dec((_,t),_)) => t ) (getFunArgs f)
                           val rtpopt = getFunRetp f
                           val rtp    = case rtpopt of
                                          SOME tt => tt
                                        | NONE    => raise Error("in Map: Function "^fid^
                                                                 " Lacks a Return Type!", pos)
                       in  if ((length atps) = 1) andalso (isBasicType rtp) andalso (isBasicType (hd atps))
                           then let fun procEl( i : int ) : BasicVal =
                                          let val v = BVal(Array.sub(arr,i))
                                          in case callFun( getFunDec f, [v], [Literal(v,pos)], vtab, ftab, pos ) of
                                               SOME (BVal v) => v
                                             | SOME x        => raise Error("in Map: resarray element " ^
                                                                            " Is Not a basic value "^pp_val x, pos )
                                             | NONE          => raise Error("in Map: no result val " ^ fid, pos)
                                          end
                                    val sz    = (hd shape) * (hd strides)
                                    val rarr = Array.tabulate(sz, procEl)
                                in  Arr(rarr, shape, strides, map_res_tp)
                                end
                           else raise Error("in evalExp of Map: illegal function argument "^fid^" at: ", pos)
                       end
        end
